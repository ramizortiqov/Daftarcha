...
//import com.or.daftarcha.data.model.ProjectEmployeeWorkdays
//import com.or.daftarcha.data.model.ProjectTotalWorkdays
...
import com.or.daftarcha.data.dao.AppUserDao
...
class EmployeeDetailViewModel @Inject constructor(
    ...
    // Ушбу шерик бириктирилган объектлар — "Пул бериш" пайтида қайси объект
    // кассасидан чиқарилаётганини танлаш учун.
    val employeeProjects: StateFlow<List<Project>> = employeeId.flatMapLatest { id ->
        if (id > 0) {
            projectEmployeeDao.getProjectIdsForEmployee(id).flatMapLatest { ids ->
                if (ids.isEmpty()) flowOf(emptyList()) else projectDao.getProjectsByIds(ids)
            }
        } else {
            flowOf(emptyList())
        }
     ...
     * Касса объект бўйича ҳисобланади, шунинг учун қайси объект кассасидан
     * пул чиқарилаётгани кўрсатилиши шарт (agar kassa yetmasa ham chiqariladi,
     * lekin kassa manfiy bo'lib qoladi).
     */
    fun addPayment(amount: Double, description: String?, projectId: Int) {
        // Ensure valid employee ID, project ID and positive amount
        if (employeeId.value == 0 || amount <= 0 || projectId <= 0) return
        viewModelScope.launch {
